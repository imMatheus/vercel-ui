...
```tsx /tw-/
<AlertDialog className="tw-grid tw-gap-4 tw-border tw-bg-background-200 tw-shadow-lg" />
```
...
```tsx /bg-background-200/ /text-foreground/
<div className="bg-background-200 text-foreground" />
```
...
>(({ className, ...props }, ref) => (
  <SheetPrimitive.Overlay
    className={cn(
      "fixed inset-0 z-50 bg-background-200/80 backdrop-blur-sm data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0",
      className
    )}
    {...props}
    ref={ref}
